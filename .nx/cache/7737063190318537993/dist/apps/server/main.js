...
let AppService = class AppService {
    async getPeople(page) {
        try {
            // Fetch the list of people from SWAPI
            const response = await axios_1.default.get(`https://www.swapi.tech/api/people?page=${page}`);
            ...
            for (const person of people) {
                ...
                // Parse the person details according to your schema
    ...
    isValidUrl(url) {
        try {
            // Validate if the URL is absolute
            if (!/^https?:\/\//.test(url)) {
